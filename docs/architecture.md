...
default_share_with       = ["desktop", "server"]
...
The `[discovery]` section is covered in depth below. Peers can be declared
here so Home Manager/Nix can generate the mesh topology. Imperative
`dotkeeper peer add` entries in `state.toml` are merged with these declarative
peers. Everything else here is a default that repos can override in their own
`dotkeeper.toml`.
...
- This machine's derived Syncthing device ID
- Imperative peer entries added with `dotkeeper peer add`
...
The Syncthing private key lives in dotkeeper's runtime Syncthing config
directory, not in `machine.toml`, because files in the Nix store are
world-readable. Anything home-manager touches could land in `/nix/store`; a
private key there would be a serious leak.
...
```toml
[sync]
syncthing_folder_id = "dotkeeper-abc123"   # stable; generated by dotkeeper track
share_with          = ["desktop", "laptop", "server"]
                      # empty = inherit machine default; machine default empty = all known peers

[commit]
policy       = "on-idle"   # manual | on-idle | timer
idle_seconds = 300

[git_backup]
interval   = "daily"       # inherits machine default if omitted
skip_slots = [2]           # slots that should not push this repo
```
...
| `dotkeeper reconcile` | Force a reconcile pass now | `dotkeeper pair` + `dotkeeper sync` |
...
| `dotkeeper peer add/list/remove` | Manage imperative peers in `state.toml` | `dotkeeper pair` |
| `dotkeeper track <path>` | Bootstrap `dotkeeper.toml` and register a repo outside scan roots | `dotkeeper add` (for out-of-root paths) |
