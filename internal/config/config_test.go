...
func TestAddMachine(t *testing.T) {
	...
	AddMachine(cfg, "laptop", "example-laptop", 1, "DEVICE-ID")
	if len(cfg.Machines) != 1 {
		t.Fatalf("expected 1 machine, got %d", len(cfg.Machines))
	}
	if cfg.Machines["laptop"].Hostname != "example-laptop" {
		t.Errorf("hostname = %q, want %q", cfg.Machines["laptop"].Hostname, "example-laptop")
	}

	// Update existing
	AddMachine(cfg, "laptop", "example-laptop", 1, "NEW-DEVICE-ID")
