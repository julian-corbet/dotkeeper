...
func TestRepoV2RoundTrip(t *testing.T) {
	...
	if len(loaded.Sync.ShareWith) != 1 || loaded.Sync.ShareWith[0] != "laptop" {
		t.Errorf("Sync.ShareWith = %v, want [laptop]", loaded.Sync.ShareWith)
	}
...
func TestStateV2RoundTrip(t *testing.T) {
	...
	original := &StateV2{
		...
		Peers: []PeerEntry{
			{Name: "laptop", DeviceID: "EEEEEEE-LLLLLLL-1111111-2222222-3333333-4444444-5555555-6666666", LearnedAt: now},
		},
	...
	if loaded.Peers[0].Name != "laptop" {
		t.Errorf("Peer name = %q, want laptop", loaded.Peers[0].Name)
	}
